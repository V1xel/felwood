...
A catalog is a top-level namespace that points to a data source. The full path to a table is `catalog.database.table`.

Doris has two kinds:
- **Internal catalog** (`internal`) — the default, stores Doris-native tables
- **External catalogs** — connect to outside data sources without copying data (Hive, Iceberg, MySQL, PostgreSQL, Elasticsearch, etc.)
...
SELECT * FROM hive.sales_db.orders;
--             ^      ^         ^
--          catalog  database  table
```

The same `catalog.database.table` convention exists in PostgreSQL (`pg_catalog.pg_tables`) and MySQL (`information_schema.tables`).

Felwood's `Catalog` class is not this concept — it is closer to what Doris calls a database (a named collection of tables within one data source).
...
In Doris the IDs are generated by the FE from a global allocator that starts at a high base (e.g. 10000) to avoid collisions with reserved system IDs. Tablet IDs include a schema hash suffix, and rowset IDs are UUID-like. The result looks like:

```
data/
└── 10001/              ← database id
    └── 10032/          ← table id
        └── 10033_0/    ← tablet id (tablet_id + schema_hash)
            └── 020000000000001234.../  ← rowset id (UUID-like)
                └── 0.dat
```

Felwood uses simple incrementing integers (1, 2, 3) with a `catalog.dat` file mapping names to IDs. The principle is the same; the format doesn't matter at single-node scale.
